...
build:
	go build -ldflags "-s -w -X github.com/kahgeh/lemonade/lemon.Version=$(VERSION)"

install:
	go install -ldflags "-s -w -X github.com/kahgeh/lemonade/lemon.Version=$(VERSION)"

release:
	gox --arch 'amd64 386' --os 'windows linux darwin' --output "dist/{{.Dir}}_{{.OS}}_{{.Arch}}/{{.Dir}}" -ldflags "-s -w -X github.com/kahgeh/lemonade/lemon.Version=$(VERSION)"
